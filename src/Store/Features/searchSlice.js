...
export const record_movie_history = createAsyncThunk(
    ...
    async ({ movie_name }, { rejectWithValue }) => {
      try {
        const response = await axios.post(`${base_url}/record_user_history`, {
          movie_name,
        });
        return username;
      } catch (error) {
...
export const on_change_search = createAsyncThunk(
    ...
    async ({ search_input }, { rejectWithValue }) => {
      try {
        const response = await axios.post(`${base_url}/on_change_search`, {
            search_input,
        });
        return username;
      } catch (error) {
  ...
  async ({  username,input,rememberMe}, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `${base_url}/user_searches?username=${username}`
      );
